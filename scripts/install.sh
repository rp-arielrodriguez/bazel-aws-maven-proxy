...
INSTALL_DIR="${INSTALL_DIR:-$HOME/.bazel-aws-maven-proxy}"
...
if [ ! -d "$INSTALL_DIR" ]; then
  ...
  echo "Re-install: curl -sL .../install.sh | bash" >&2
  exit 1
fi

cd "$INSTALL_DIR"

# Map subcommands to mise tasks
CMD="${1:-help}"
...
case "$CMD" in
  ...
  help|-h|--help)
    ...
    echo ""
    echo "Config:"
    echo "  config             Show current .env values"
    echo "  config:set K=V     Update .env setting"
